# Cookbook Overview

- [Filters](cookbook/filters.md)
    - [How to override filter label](cookbook/filters.md#how-to-override-filter-label)
    - [How to create a custom filter](cookbook/filters.md#how-to-add-a-custom-filter)
    - [How to remove an unwanted filter](cookbook/filters.md#how-to-remove-an-unwanted-filter)
    - [How to predefine often used filters](cookbook/filters.md#how-to-predefine-often-used-filters)

- [Search](cookbook/search.md)
    - [Prerequisites](cookbook/search.md#prerequisites)
    - [Search on Definition](cookbook/search.md#search-on-definition)
    - [Global Search](cookbook/search.md#global-search)
    - [Configuration](cookbook/search.md#configuration)

- [Custom Styles](cookbook/custom_styles.md)
    - [Style Hooks](cookbook/custom_styles.md#style-hooks)
    - [Script Hooks](cookbook/custom_styles.md#script-hooks)
    - [What needs to be done?](cookbook/custom_styles.md#what-needs-to-be-done)
    - [What can be used within the main style](cookbook/custom_styles.md#what-can-be-used-within-the-main-style)
    - [Naming conventions](cookbook/custom_styles.md#naming-conventions)
    - [There's no hook for what I want to style](cookbook/custom_styles.md#there39s-no-hook-for-what-i-want-to-style)
